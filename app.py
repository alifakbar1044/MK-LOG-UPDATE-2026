...
def export_pdf():
    ...
    # Lebar kolom (update kolom 'Nama' jadi 50)
    width_no = 10
    width_nama = 50
    width_kelas = 25
    width_tanggal = 30
    ...
    total_width = width_no + width_nama + width_kelas + width_tanggal + width_waktu + width_keterangan
    start_x = (210 - total_width) / 2  # biar tabel tetap di tengah

    # Header tabel
    pdf.set_font("Arial", 'B', 10)
    pdf.set_x(start_x)
    pdf.cell(width_no, 10, "NO", 1, 0, 'C')
    pdf.cell(width_nama, 10, "Nama", 1, 0, 'C')
    pdf.cell(width_kelas, 10, "Kelas", 1, 0, 'C')
    pdf.cell(width_tanggal, 10, "Tanggal", 1, 0, 'C')
    pdf.cell(width_waktu, 10, "Waktu", 1, 0, 'C')
    pdf.cell(width_keterangan, 10, "Keterangan", 1, 1, 'C')
    ...
    for row in rows:
        pdf.set_x(start_x)
        pdf.cell(width_no, 10, str(no), 1, 0, 'C')

        pdf.set_font("Arial", size=9)
        pdf.cell(width_nama, 10, str(row.get('nama', '')), 1, 0, 'C')
        pdf.set_font("Arial", size=12)

        pdf.cell(width_kelas, 10, str(row.get('kelas', '')), 1, 0, 'C')
        pdf.cell(width_tanggal, 10, str(row.get('tanggal', '')), 1, 0, 'C')
        pdf.cell(width_waktu, 10, str(row.get('waktu', '')), 1, 0, 'C')
        pdf.cell(width_keterangan, 10, str(row.get('keterangan', '')), 1, 1, 'C')
